...
comment on column xxl_job_log_report.id is '报表主键';
...
alter table xxl_job_log_report add constraint PK_XXL_JOB_REPORT_ID primary key (id);
alter table xxl_job_log_report add constraint UN_XXL_JOB_REPORT_DAY unique (trigger_day);
...
create index IDX_XXL_JOB_UPDATE_TIME on XXL_JOB_REGISTRY (UPDATE_TIME)
    tablespace TS_MSUNERP
    pctfree 10
    initrans 2
    maxtrans 255
